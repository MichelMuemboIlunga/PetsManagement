...
if (isset($_POST['reg_btn'])) {
    ...
    $check_user = "SELECT * FROM tUser WHERE username='$username' OR email='$email' LIMIT 1";
    ...
    if (count($errors) == 0) {
        ...
        $statement = "INSERT INTO tUser (username, email, password)
...
if (isset($_POST['login'])) {
    ...
    if (count($errors) == 0) {
        ...
        $statement= "SELECT * FROM tUser WHERE email='$email' AND password='$password'";
        ...
        if (mysqli_num_rows($output) == 1) {
            ...
            // fetch data
            $uid = mysqli_fetch_array($output);
            echo $uid;



            $_SESSION['name'] = $uid['username'];
            $_SESSION['email'] = $email;
